...
public class AngryFlappyBird2 extends Application {
    
    private Defines2 DEF = new Defines2();
    private Sound2 sound = new Sound2();
    private Score2 SCORE = new Score2();
    ...
    // game components
    private Sprite2 blob;
    private ArrayList<Sprite2> floors;
    private ArrayList<Sprite2> pipeUps;
    private ArrayList<Sprite2> pipeDowns;
    private ArrayList<Sprite2> breads;
    private ArrayList<Sprite2> peaches;
    private ArrayList<Sprite2> eggs;
    ...
    private boolean HIT_PIPE_OR_PIG;
    ...
    void resetGameScene(boolean firstEntry) {   
        ...
        for(int i=0; i<DEF.FLOOR_COUNT; i++) {          
            ...
            Sprite2 floor = new Sprite2(posX, posY, DEF.IMAGE.get("floor"));
        ...
        // initialize blob
        blob = new Sprite2(DEF.BLOB_POS_X, DEF.BLOB_POS_Y,DEF.IMAGE.get("kiki01"));
        blob.render(gc);
        
        // initialize pipeUp  
        Random ran = new Random();
        Sprite2 pipeUpCopy = new Sprite2(0, 0, DEF.IMAGE.get("pipeflap2"));
        for (int i = 0; i < DEF.PIPE_COUNT; i++) {
            int PIPEUP_POS_X = ran.nextInt((i+1)*250, (i+2)*250);
            int PIPEUP_POS_Y = ran.nextInt(-40, -30);            
            Sprite2 pipeUp = new Sprite2(PIPEUP_POS_X, PIPEUP_POS_Y, DEF.IMAGE.get("pipeflap2"));
            if (pipeUpCopy.intersectsSprite(pipeUp)) {
                pipeUp = new Sprite2(PIPEUP_POS_X + 150, PIPEUP_POS_Y, DEF.IMAGE.get("pipeflap2"));
            }
            pipeUpCopy = pipeUp;
        ...
        // initialize pipeDown
        Sprite2 pipeDownCopy = new Sprite2(0, 0, DEF.IMAGE.get("pipeflap"));
        for (int i = 0; i < pipeUps.size(); i++) {
            ...
            Sprite2 pipeDown = new Sprite2(PIPEDOWN_POS_X, PIPEDOWN_POS_Y, DEF.IMAGE.get("pipeflap"));
            if (pipeDownCopy.intersectsSprite(pipeDown)) {
                pipeDown = new Sprite2(PIPEDOWN_POS_X + 150, PIPEDOWN_POS_Y, DEF.IMAGE.get("pipeflap"));
            }
        ...
        for (int i=0; i < pipeDowns.size() ; i++) {
            Sprite2 peach = new Sprite2();
            Sprite2 egg = new Sprite2();
        ...
        int counter = 0;    
         ...
         public void checkCollision() {     
            ...
            if (!isSnoozed) {
              ...
              for (Sprite2 pipe : pipeUps) {
                  if (blob.intersectsSprite(pipe)) {
                      HIT_PIPE_OR_PIG = true;
                   }
              }               
              for (Sprite2 pipe : pipeDowns) {
                  if (blob.intersectsSprite(pipe)) {
                      HIT_PIPE_OR_PIG = true;
                   }            
               }           
              for (Sprite2 bread : breads) {
